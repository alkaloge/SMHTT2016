...
fileQCD=ROOT.TFile("files_control/QCD.root","r")
fileW=ROOT.TFile("files_control/W.root","r")
...
for i in range(0,len(plots)):
   ...
   QCD=fileQCD.Get(categories[i]).Get(str(plots[i])+"QCD")
   W=fileW.Get(categories[i]).Get(str(plots[i])+"W") 
   #QCD.Scale(0.000001)
   ...
   c.Modified()
   c.SaveAs("new_control_mt/"+str(plots[i])+".pdf")
   c.SaveAs("new_control_mt/"+str(plots[i])+".png")
